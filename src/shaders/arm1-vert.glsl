...
void main()
{
    ...
    if (fs_Pos.y < 0.f) {
        fs_Pos.y *= easeInQuadratic(abs(fs_Pos.y)); //1.25f;
    }
    if (fs_Pos.y < -0.7f) {
        fs_Pos.y *= bias(abs(fs_Pos.y) + 0.1f * noisyValue, 0.9f);
    }
    fs_Pos.y *= 1.2f;

    fs_Pos.x *= bias(noisyValue, 0.6f);
    fs_Pos.y *= bias(noisyValue, 0.6f);
    fs_Pos.z *= bias(noisyValue, 0.6f);
    ...
    fs_LightVec = lightPos - modelposition - vec4(10.f, 50.f, 5.f, 1.f);  // Compute the direction in which the light source lies
